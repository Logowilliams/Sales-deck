...
online <- mutate(online, `POS Sales` = 0)
...
```{r}
#create column index
cols.num.inv <- c(1,6:13)
#prep for merge, make sure columns have same class
inventory[cols.num.inv] <- sapply(inventory[cols.num.inv],as.numeric)
inventory%>%
  group_by(`Item Nbr`)%>%
  summarise(onHand = sum(`Curr Str On Hand Qty`), instock = mean(`Curr Instock %`)) -> inventory
```

# Check for New UPCs
```{r}
upcList <- read_sheet(params$gtmTracker, sheet = "WMT SKU Sales [2023]", range = "B3:B177")
